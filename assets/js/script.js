  ...
  /* ЗМІНА 1b: "Close" hint badge — periodic, unobtrusive fade reminder
    that the open strip can be closed. Purely visual, no logic change
    to open/close behaviour. */
  let _closeHintInterval = null;
  let _closeHintShowTimeout = null;

  function startCloseHint(strip) {
    stopCloseHint();
    const hint = strip.querySelector(".fstrip-hint--close");
    if (!hint) return;

    const flash = () => {
      hint.classList.add("is-visible");
      _closeHintShowTimeout = setTimeout(
        () => hint.classList.remove("is-visible"),
        2200,
      );
    };

    _closeHintShowTimeout = setTimeout(flash, 1200);
    _closeHintInterval = setInterval(flash, 7000);
  }

  function stopCloseHint() {
    if (_closeHintInterval) clearInterval(_closeHintInterval);
    if (_closeHintShowTimeout) clearTimeout(_closeHintShowTimeout);
    _closeHintInterval = null;
    _closeHintShowTimeout = null;
    document
      .querySelectorAll(".fstrip-hint--close.is-visible")
  ...
  function initFstripScrollHint(panel) {
    ...
    observer.observe(panel.closest(".film-strip"), {
      attributes: true,
  ...
  (function initProductionFilters() {
    ...
    */
    ...
    function applyFilters() {
      ...
      const seeMoreWrap = seeMoreBtn
        ? seeMoreBtn.closest(".see-more-wrap")
        : null;
      if (seeMoreWrap) {
        const hiddenVisibleCount = Array.from(items).filter(
          (item) =>
            !item.classList.contains("pf-hidden") &&
    ...
    function itemMatchesAllFilters(item) {
      for (const [groupName, activeVal] of Object.entries(activeFilters)) {
        ...
        const itemValues = (item.dataset[attrKey] || "")
          .trim()
          .toLowerCase()
          .split(/\s+/);
  ...
  (function () {
    ...
    function attachToAccordions() {
      ...
      document
        .querySelectorAll(".film-acc-item.film-acc-open .film-stills-slider")
        .forEach(initSlider);
    }
